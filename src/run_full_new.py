...
def demo_images():
    ...
    binTh = 0.85  # final thresholding to obtain mask
    ...
    for imdir in imDirs:
        ...
        interval = 5
        shot_len = len(imPathList)
#        print('len:',shot_len)
        shots_num = math.floor(float(shot_len)/float(interval))
        #print('shots_num:',shots_num)
        #print('imSeq:',np.shape(imSeq))
        imSeq_edit_ori = imSeq
        # print('imSeq.shape:',np.shape(imSeq_ori))
        
        # a = []

       # print('imSeq',np.shape(imSeq_edit))
        for ind_shot in range(int(shots_num)):
            if ind_shot == shots_num-1:
                shot_begin = int(ind_shot*interval)
                print("b",shot_begin)
                shot_end = int(shot_len)
                print("e",shot_end)
            else:
                shot_begin = int(ind_shot*interval)
                print("b",shot_begin)
                shot_end = int((ind_shot+1)*interval)
                print("e",shot_end)

            imSeq_edit = imSeq_edit_ori[shot_begin:shot_end]
            print('number of frames on current shot!!!:',shot_end-shot_begin)
            print('imSeq[0]',np.shape(imSeq_edit))

            # First run shot detector
            if not doload:
                shotIdx = vid2shots.vid2shots(imSeq_edit, maxShots=maxShots, vmax=vmax,
                                                colBins=colBins)
                if dosave:
            ...
            for s in range(shotIdx.shape[0]):
                ...
                shotE = imSeq_edit.shape[0] if s == shotIdx.shape[0] - 1 \
                ...
                imSeq1 = imSeq_edit[shotS:shotE:frameGapLocal + 1]

                print('shotS:',shotS)
                print('shotE:',shotE)
                print('frameGapLocal',frameGapLocal)
                ...
                # print('maskSeq', maskSeq)
                # print('maskSeq_type',type(maskSeq))
                ...
                # print('img_combine',np.shape(img_combine))
            # mask_combine = np.concatenate(mask_combine, maskSeq)
            if ind_shot == 0:
                mask_combine = maskSeq
                img_combine = imSeq1
            # print('maskSeq:',np.shape(maskSeq))
            else:
                mask_combine = np.concatenate((np.array(mask_combine),maskSeq),axis = 0)
                img_combine = np.concatenate((img_combine,imSeq1),axis=0)

            # print('maskcombine:',np.shape(mask_combine))
            # mask_combine.append(maskSeq)
            # mask_combine = np.array(mask_combine)
            # img_combine.append(imSeq1)
            
            # print('mask_combine:', mask_combine)
            # print('mask_combine_shape:',np.shape(mask_combine))
            # print('mask_shape_type:',type(mask_combine))

            # save as video
        sTime = time.time()
        vidName = '_'.join(imdir.split('/')[3:]) + '_shot%d.avi' % (s + 1)

        # print('shape:',np.shape(img_combine))
        # print('shape2',np.shape(mask_combine))
        img_combine = np.array(img_combine)
        print('img_combine:',np.shape(img_combine))
        utils.im2vid(outVidNlc + vidName, img_combine,(np.array(mask_combine) > binTh).astype(np.uint8))
#        utils.im2vid(outVidCRF + vidName, img_combine, crfSeq)
        eTime = time.time()
